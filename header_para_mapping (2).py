        ...
        try:
            ...
            if headers and sections:
                for i, header in enumerate(headers):
                    ...
                    header_para_map[header_text] = section_paragraphs
            
            # If sections-based approach doesn't work, try direct paragraph mapping
            if not header_para_map and headers and paragraphs:
                # Simple approach: map each header to all paragraphs that follow it
                ...
                for line in lines:
                    line = line.strip()
                    if line.startswith('#'):
                        # Save previous header's paragraphs
                        if current_header and current_paras:
                            header_para_map[current_header] = current_paras.copy()
                    ...
                    elif not line and current_paras:
                        # Empty line - group paragraphs
                        if current_paras:
                            para_text = ' '.join(current_paras)
                            if current_header:
                                if current_header not in header_para_map:
                                    header_para_map[current_header] = []
                                header_para_map[current_header].append(para_text)
                            current_paras = []
